...
class Meccadumps_CvvFinder:
    ...
    def login(self):
        ...
        while(True):
            ...
            try:
                ...
                #captch.clear()
                #elm = self.driver.find_element_by_css_selector('#loginForm > div.captcha-container.text-center > div > img')
                #elm.click()
                continue
            except Exception as e:
    ...
    def get_current_page_cvvs(self):
        ...
        try: 
            ...
            self.wait.until(EC.visibility_of_all_elements_located((By.CLASS_NAME,'city-cell')))
        ...
        try:
            ...
            for row in rows:
                ...
                # is_refund
                is_refund = row.find('div',{'class':'l-refcc'}).get_text().strip()
                #print(is_refund)
                ldatecc = row.find('div',{'class':'l-exp'}).get_text().strip()
                ...
                #print(card_mark)
                #owner name
                name = row.find('div',{'class':'l-name'}).get_text().strip()
                city_cell =row.find('td',{'class':'city-cell'})
                city = city_cell.find('div',{'class':'l-city'}).get_text().strip()
                zip_no = city_cell.find('div',{'class':'l-zip'}).get_text().strip()
                #print(zip_no)
                #state = row.find('td',{'class':'lstate'}).get_text().strip()
