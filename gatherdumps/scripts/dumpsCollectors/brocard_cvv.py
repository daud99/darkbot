...
class Brocard_CvvFinder:
    ...
    def login(self):
        while(True):
            ...
            try:
                ...
                self.wait.until(EC.visibility_of_element_located((By.XPATH,'/html/body/div[2]/ul/li[4]/a')))
    ...
    def visit_cvvs(self):
        self.driver.load_url(self.url+'cvv',wait_for_page_body=True)
    ...
    def get_current_page_cvvs(self):
        ...
        try: 
            ...
            self.wait.until(EC.visibility_of_all_elements_located((By.CLASS_NAME,'js-card-row')))
        except TimeoutException as te:
            return None
        ...
        try:
            ...
            rows = bs.find_all('tr',{'class':'js-card-row'})
            for row in rows:
                lbincc = row.find('td',{'class':'lbincc'})
                bin_no = lbincc.find('div',{'class':'l-bin'}).get_text().strip()
            
                lcard_type = lbincc.find_next_sibling('td')
                #category
                ...
                # is_refund
                is_refund = row.find('td',{'class':'lrefund'}).get_text().strip()
                
                #print(is_refund)
                card_mark = row.find('td',{'class':'lmarkcc'}).get_text().strip()
                #print(card_mark)
                #owner name
                name = row.find('td',{'class':'lname'}).get_text().strip()
                city_td =row.find('td',{'class':'lcity'})
                city = city_td.find('div',{'class':'l-city'}).get_text().strip()
                state = city_td.find('div',{'class':'lstate'}).get_text().strip()
                zip_no_td =row.find('td',{'class':'lzip'})
                ldatecc = zip_no_td.find('div',{'class':'ldatecc'})
                ldatediv = ldatecc.find('div',{'class':'l-exp'}).get_text().strip()
                expiry = str(ldatediv)
                zip_no = zip_no_td.find('div',{'class':'l-zip'}).get_text().strip()
                card_bank =row.find('td',{'class':'lbankcc'})
                card_bank = card_bank.find('div',{'class':'l-bank'}).get_text().strip()
                #print(card_bank)
                lcountry = row.find('td',{'class':'lcountrycc'})
                '''
                lcountry.div.clear()
                ...
                base = base.find('div',{'class':'l-base'}).get_text().strip()
                #print(base)
                
                #print(quantity)
                price = row.find('td',{'class':'lpricecc'})
                ...
                bank_json['expiry'] = expiry
                bank_json['name']= name
                bank_json['city']= city
                bank_json['state']= state
                bank_json['zip_no']= zip_no
