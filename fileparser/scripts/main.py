...
def main(folder_path=None):
    ...
    if fsi_object == None:
        ...
        return
    print(fsi_object)
    print(fsi_object.folder_path)
    fri_object = FileReadInfoModel.objects.get(folder=fsi_object)
    if fri_object == None:
        updateFSRStatus(folder_path, False)
        return
    # here we have to add the logic for selecting the appropriate parser based on parser value
        ...
        try:
            ...
            if groups:
                ...
                if len(groups) == len(additional_field):
                    p = TextFileParser(each, fri.length, fri.order, fri.delimeter, fri.start_line, groups, additional_field)
                else:
                    p = TextFileParser(each, fri.length, fri.order, fri.delimeter, fri.start_line)
                p.readFile()
            else:
                print("wao")
                p = TextFileParser(each, fri.length, fri.order, fri.delimeter, fri.start_line)
